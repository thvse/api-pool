...
- 👁️ **智能多模态视觉转译 (Vision Translation)**：
  如果客户端发送了携带图片的多模态请求，但命中的高优端点不支持视觉能力（如纯文本模型），系统将自动从池内**调度带视觉能力的模型**（如 GPT-4o, GLM-4V）执行前置图像解析任务。解析后的详细文字描述将被天衣无缝地拼接到上下文中，进而继续传递给纯文本模型处理。控制台列表亦支持通过 UI 徽章直观显示节点的 Vision 视觉解析支持状态。
- 🔌 **全双工异构协议兼容**：
...
- 🗂️ **情景化悬浮测试舱 (Contextual Test Drawer)**：
  原生集成极简风交互体验，丢弃臃肿的静态面板，采用右下角按需唤出的玻璃态悬浮抽屉进行端点连通性测试及图片视觉测试，支持无缝切换端点覆盖测试信息。
- 📦 **纯原生 零依赖**：
